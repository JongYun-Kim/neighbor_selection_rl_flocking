"""Dormant heuristic baselines, moved out of baselines.py.

Nine of the twelve heuristics in the original baselines.py — about 1,800 of its
2,100 lines — are referenced only by test_baselines.py and the verify_*.py
scripts in this directory. The three the active code and the studies actually
use (random / distance / nearest) stayed behind. These classes are unchanged
and are re-exported by baselines.py, so `from baselines import <anything>` and
`create_baseline(<any type>)` keep working exactly as before:

  VoronoiNeighborSelection               'voronoi'
  FixedFarthestNeighborSelection         'farthest'
  MetricTopologicalInteractionSelection  'mti'
  HighestDegreeNeighborSelection         'highest_degree'
  ModifiedFixedNumberNeighbors           'mfnn'
  ActiveSearchNeighborSelection          'active_search'
  GazingPreferenceNeighborSelection      'gazing_preference'
  MotionSalienceThresholdSelection       'motion_salience'
  VisualAttentionNeighborSelection       'visual_attention'
"""
