...
Defined here, the three the active code and the studies use:
...
The other nine (voronoi, farthest, mti, highest_degree, mfnn, active_search,
gazing_preference, motion_salience, visual_attention) live in
legacy/baselines_extra.py and are re-exported at the bottom of this module, so
importing them from here and building them with create_baseline both work
unchanged.
...
# The dormant nine, re-exported so that every existing import site and the
# create_baseline registry above resolve them exactly as before. Placed at the
# bottom because create_baseline looks its classes up at call time.
from legacy.baselines_extra import (  # noqa: E402
