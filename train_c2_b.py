"""Variant B ("threshold") training under the C2 convergence criterion.

Study: studies/acs-c2-train (see PROBLEM.md/PLAN.md there).
Per-agent learned-threshold selection (adaptive degree) WITH an annealed
distance-rank prior (dist_aux 1.0 -> 0.2 over 400k env steps, then hold).
Env: C2 success-only early termination + c2_shaping reward + global_stats obs.
GPU: cuda:3 (user-assigned).

Usage:
    CUDA_VISIBLE_DEVICES is forced to "3" below (before ray/torch import).
    python train_c2_b.py            # full run (100 iters, GPU)
    python train_c2_b.py --smoke    # 2-iter CPU smoke test (Phase 2 gate)
"""
import argparse
import os

# Pin to GPU 3 for this experiment (variant A uses GPU 1). Must be set BEFORE
...
RUN_NAME = "c2B3_threshold_biasinit_260806"
...
# Variant B model: per-agent threshold head + annealed distance-rank prior.
CUSTOM_MODEL_CONFIG = {
    "d_embed_context": 128,
    "d_embed_input": 128,
    "d_ff": 256,
    ...
    # --- variant B head ---
    "selection_head": "threshold",
    "logit_scale": 10.0,
    # B3: start the bar BELOW the att distribution (dense sampling p~0.9,
    # argmax ~ fully-connected). B (tau=0) and B2 (tau=0 + entropy anneal)
    # both left the argmax readout degenerate-sparse (deg 2-3, eval 0/16
    # through iters 10-50): with p pinned ~0.5, per-edge PG is advantage-noise
    # dominated and tau never finds a useful split. Starting from the
    # known-feasible dense end makes pruning the learned direction.
    "threshold_bias_init": -0.1,
    "top_k": None,
    "hard_top_k": False,
    "dist_aux_coef": 1.0,
    "dist_aux_k": 10,
...
def build_tune_config(smoke: bool) -> dict:
    config = {
        ...
        "kl_target": 0.01,
        "entropy_coeff": 1e-3,
        # Anneal the entropy bonus: run c2B_threshold_260806 (iters 1-31) kept
        # per-edge p pinned at ~0.5 (bonus ~+0.26/step vs settled shaping
        # ~-0.003/step), so the deterministic argmax readout stayed degenerate-
        # sparse (deg~2-3, eval 0/16 at iters 10/20/30) while sampling
        # succeeded densely. Keep early exploration, then let margins open.
